...
class AttackerCapabilities(LoggingClass):
    ...
    def update_capabilities(self, new_time, new_dst, logins):
        ...
        for (user) in compromised_users:
            self._update_candidate_dst(logins, user, None)

        compromised_users = set([u for (u) in compromised_users])
    ...
    def _get_new_compromised_users(self, logins, time, host):
        ...
        lower_bound = time - datetime.timedelta(hours=self.compromise_cred_hrs).total_seconds()
        vuln_logins = logins[
        ...
        # ]
        vuln_users = [
            (r[LoginColumns.USER]) for idx, r in
            vuln_logins[
                [LoginColumns.USER]
            ].drop_duplicates().iterrows()
        ]
        ...
        compromised_users = set([
            (u) for (u) in vuln_users])
        print(len(compromised_users))
