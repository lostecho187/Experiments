...
class AttackPathGenerator(LoggingClass):
    ...
    def make_attack(self, logins):
        ...
        print('###1, time:', time.time() - ini)

        # Initialize starting hop
        ini = time.time()
        self._initialize_start(logins)
        print('###2, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        ...
        while not self.attack_goal.is_attack_complete(self.attack_history):
            ...
            print('###3, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
            ...
            print('###4, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))

            ini = time.time()
            new_hop.loc[:, LoginColumns.ATTACK] = True

            # Make the lateral move to the new destination & update state
            print('###5, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
            ini = time.time()
            self._make_next_hop(new_hop, logins)
            print('###6, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
    ...
    def _make_next_hop(self, new_hop, logins):
        ...
        print('###61, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        compromised_users = self.attack_capabilities.update_capabilities(
            new_time, new_dst, logins)
        print('###62, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        self.attack_stealth.update_knowledge(new_time, new_dst, logins)
        print('###63, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        self.attack_history.add_new_hop(new_hop)
        print('###64, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        self.attack_history.update_compromised_creds(new_dst, compromised_users)
        print('###65, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))
        self.attack_goal.update_progress(new_dst, compromised_users)
        print('###66, time:', time.time() - ini, ', attack path:', self.attack_history.attack_path.reset_index(drop=True))

    def _get_next_hop(self, logins):
        """HELPER Method: Select the next attack hop to make."""
        print(self.attack_history)
        ...
        next_time = (
            ...
            datetime.timedelta(seconds=next_interarrival).total_seconds()
        )
        ...
        candidate_next_hops = self.attack_capabilities.get_candidate_next_hops(
            self.attack_history)
        print('###terminating1', type(candidate_next_hops), len(candidate_next_hops))
        ...
        # print('terminating2', candidate_next_hops)
        ...
        candidate_next_hops = self.attack_constraints.constrain_next_hops(
            candidate_next_hops, self.attack_history)
        # print('terminating3', candidate_next_hops)
        ...
        # print('terminating4', candidate_next_hops)
