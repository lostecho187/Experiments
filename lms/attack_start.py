...
def get_viable_start_users(logins, target_dsts=set([])):
    ...
    direct_access_users = set(logins[LoginColumns.USER].drop_duplicates())
    ...
    candidate_users = set(pd.read_csv('./attackers.txt', header=None)[0])
...
class AttackStart(object):
    ...
    def time_box_logins_for_user_src(cls, logins, start_user, start_src, min_time=None):
        ...
        max_time = logins[
            (logins[LoginColumns.SRC] == start_src) &
            (logins[LoginColumns.USER] == start_user)
        ][LoginColumns.TIME].max() + int(datetime.timedelta(days=7).total_seconds() / 3600)
    ...
    def _select_start_time_confusion_paths(self, logins):
        ...
        self.start_time = self.start_time + int(datetime.timedelta(seconds=rand_seconds).total_seconds() / 3600)
    ...
    def _select_start_user_random(self, logins, candidate_start_users=None):
        ...
        if not candidate_start_users:
            candidate_start_users = set(logins[LoginColumns.USER].drop_duplicates()) #self._get_viable_start_users(logins)
        self.start_user = random.sample(list(candidate_start_users), 1)[0]
    ...
    def _get_src_machine_for_user(self, user, logins):
        ...
        for src in top_srcs:
            ...
            if len(owners) > 0 and owners.iloc[0].user == user:
                return src
        print(top_srcs)
    ...
    def _filter_logins_for_start(self, logins):
        ...
        min_time = logins[LoginColumns.TIME].min() + datetime.timedelta(weeks=2).total_seconds()
        max_time = logins[LoginColumns.TIME].max() - datetime.timedelta(weeks=1).total_seconds()
        # min_time = logins[LoginColumns.TIME].min() + datetime.timedelta(hours=2).total_seconds()
        # max_time = logins[LoginColumns.TIME].max() - datetime.timedelta(hours=1).total_seconds()
        valid_logins = valid_logins[
            (valid_logins[LoginColumns.TIME] >= min_time) &
            (valid_logins[LoginColumns.TIME] <= max_time)
        ]
        print(
            "{} candiate logins ({} users) after "#.format(
